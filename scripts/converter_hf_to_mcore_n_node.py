...
def log_host_memory(msg):
    mem = psutil.virtual_memory()
    free, total = torch_npu.npu.mem_get_info()
    used = total - free
    print(
        f"[CHECK MEM] [{msg}] host mem used: {mem.used/(1024**3):.2f}GB | "
        f"npu mem used:{used/1024**3:.2f}GB"
    )
...
def _init_args():
    ...
    parser.add_argument("--pp_size", type=int, required=True, help="pipeline_model_parallel_size")
    parser.add_argument("--ep_size", type=int, required=True, help="expert_model_parallel_size")
...
def convert_checkpoint_from_transformers_to_megatron(
    ...
    print(f"================= {pp_rank=}, {pp_size=}, {ep_rank=}, {ep_size=}, {layer_start=}, {layer_end=}")
    ...
    for layer_idx, (layer, hf_layer) in enumerate(
        ...
        for idx, hf_expert in enumerate(hf_layer.mlp.experts):
            if os.getenv('USE_GEMM', '0') == '1':
                ...
                fc1_weight = torch.cat([hf_expert.gate_proj.weight.transpose(0, 1).contiguous(), hf_expert.up_proj.weight.transpose(0, 1).contiguous()], dim=1)
                true_npu_weight1 = layer.mlp.experts.weight1.view(num_local_experts, hidden_size, -1)     # true w1 shape during gemm forward is [num_experts, hidden_size, inter_size * 2]
                ...
                true_npu_weight2 = layer.mlp.experts.weight2.view(num_local_experts, -1, hidden_size)     # true w2 shape during gemm forward is [num_experts, inter_size, hidden_size]
...
def convert_checkpoint_from_transformers_to_megatron_dpskv3(
    ...
    print(f"====info===== {pp_rank=}, {pp_size=}, {ep_rank=}, {ep_size=}, {layer_start=}, {layer_end=}")
...
def convert_hf_to_mcore(hf_model_path, output_path, pp_size, ep_size, use_cpu_initialization=False, test=False, trust_remote_code=False):
    ...
    assert ep_size * pp_size == world_size, \
        f"ep_size({ep_size}) * pp_size({pp_size}) must equal world_size({world_size})"
    ...
    log_host_memory(f"Rank_{rank} start init")
    ...
    if os.getenv('USE_GEMM', '0') == '1':
        configs_repatch['moe_alltoall_overlap_comm'] = True
    repatch(configs_repatch)
    ...
    log_host_memory(f"Rank_{rank} after hf model init")

    pp_rank = mpu.get_pipeline_model_parallel_rank()

    # distributed convert
    if world_size > 1 and support_distributed_convert(hf_config):
        log_host_memory(f"Rank_{rank} before load true weight")
        ...
        if "DeepseekV3ForCausalLM" in hf_config.architectures:
            ...
            raise NotImplementedError(f"Distributed conversion is not supported for {hf_config.architectures} yet.")
        log_host_memory(f"Rank_{rank} after load true weight")
