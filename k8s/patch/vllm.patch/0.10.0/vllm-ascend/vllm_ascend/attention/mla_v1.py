...
class AscendMLAMetadataBuilder:
    ...
    def build(
        ...
        **kwargs,
        ...
        rotary_emb = self.runner.get_model().model.layers[0].self_attn.rotary_emb
        if not isinstance(rotary_emb, rotary_embedding.DeepseekScalingRotaryEmbedding):
            self.cos_cache, self.sin_cache = rotary_emb.cos_sin_cache.chunk(2, dim=-1)

        if self.cos_cache is None:
            self.cos_cache = rotary_emb.cos_cached
            self.sin_cache = rotary_emb.sin_cached
