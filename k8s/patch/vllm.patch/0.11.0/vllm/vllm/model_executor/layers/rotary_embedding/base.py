...
class RotaryEmbedding(CustomOp):
    ...
    def __init__(
        ...
        cache, cos_cached, sin_cached = self._compute_cos_sin_cache()
    ...
    def _compute_cos_sin_cache(self) -> torch.Tensor:
        ...
        cos_cached = torch.cat([freqs, freqs], dim=-1).cos()
        sin_cached = torch.cat([freqs, freqs], dim=-1).sin()
        cache = torch.cat((cos, sin), dim=-1)
        return cache, cos_cached, sin_cached
        #return cache
