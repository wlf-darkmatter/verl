...
class TestComm(BasrRay):
    ...
    def init_process_group(self):
        ...
        if not torch.distributed.is_initialized():
            import os
            print(f"[{os.environ['RANK']}] Connecting to {os.environ['MASTER_ADDR']}:{os.environ['MASTER_PORT']}")
            dist.init_process_group(backend, timeout=datetime.timedelta(seconds=300), init_method='env://')
